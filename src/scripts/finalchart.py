...
RANDOM_STATE = 42
TRAIN_LIMIT = 150000 
SAVE_DPI = 300 # High resolution for Thesis
...
def plot_comparison_charts():
    ...
    # CHART A: THROUGHPUT (Log Scale is CRITICAL here)
    bars = sns.barplot(x=models, y=speed_pps, ax=ax1, palette=['#4c72b0', '#55a868'])
    ax1.set_yscale("log") # <--- THE TRICK TO SHOW BOTH
    ...
    # CHART B: ACCURACY
    bars2 = sns.barplot(x=models, y=accuracy, ax=ax2, palette=['#4c72b0', '#55a868'])
    ax2.set_ylim(80, 102) # Zoom in to show the difference
...
def plot_confusion_matrix():
    ...
    # We need to reload data to get y_test and X_test
    # This ensures the split is IDENTICAL to your training run
    try:
    ...
    # PREPARE DATA (Identical logic to Sprint 6)
    cols_to_drop = ['Label']
    ...
    # Note: We turn annot=False for large matrices to keep it clean, 
    # or you can use annot=True if you have few classes.
